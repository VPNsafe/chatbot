...
⚪ /retry – Сгенерировать заново последний ответ бота
...
🎨 Генерируйте изображения из текстовых подсказок в режиме <b>👩‍🎨 Художник</b> /mode
...
2. Сделайте его <b>администратором</b>, чтобы он мог видеть сообщения (все другие права могут быть ограничены)
3. Вы великолепны!

Чтобы получить ответ от бота в чате, используйте <b>тег</b> @ или <b>ответ</b> на его сообщение.
...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = "Привет! Я бот <b>ChatGPT</b>, реализованный с использованием API OpenAI 🤖\n\n"
    ...
    async def message_handle_fn():
        ...
        if use_new_dialog_timeout:
           if (datetime.now() - db.get_user_attribute(user_id, "last_interaction")).seconds > config.new_dialog_timeout and len(db.get_dialog_messages(user_id)) > 0:
            db.start_new_dialog(user_id)
            await update.message.reply_text(f"Начинаю новый диалог из-за таймаута (<b>{config.chat_modes[chat_mode]['name']}</b> режим) ✅", parse_mode=ParseMode.HTML)
    db.set_user_attribute(user_id, "last_interaction", datetime.now())
...
async def is_previous_message_not_answered_yet(update: Update, context: CallbackContext):
    ...
    if user_semaphores[user_id].locked():
      ...
      text += "Или вы можете его отменить /cancel"
...
async def edited_message_handle(update: Update, context: CallbackContext):
   if update.edited_message.chat.type == "private":
    text = "🥲 К сожалению, редактирование сообщений <b>не поддерживается</b>"
...
async def post_init(application: Application):
    await application.bot.set_my_commands([
        ...
        BotCommand("/retry", "Сгенерировать ответ заново для предыдущего запроса"),
